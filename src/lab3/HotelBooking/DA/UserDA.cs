...
namespace HotelBooking
{
    public class UserDA : IUserRepository
    {
        ...
        {
            ConnectionCheck.checkConnection(this.Connector);
            string sql = queryUpdateUser(id, password, role);
            NpgsqlCommand cmd = new NpgsqlCommand(sql, this.Connector);
            cmd.ExecuteNonQuery();
        }
        ...
        public string queryUpdateUser(int id, string password, Role role)
        {
            return "update Users set password = '" + password + "', role = '" + role.ToString() +
